...
public class WinnerNumberGeneratorValidator {
    ...
    private void vaildWinnerNumberRange(List<Integer> winnerNumbers) {
    ...
    private void vaildWinnerNumberSize(List<Integer> randomNumbers) {
        if (randomNumbers.size() != 6) {
            throw new IllegalArgumentException("[ERROR] 당첨 번호가 6개로 생성되지 않았습니다.");
        }
    }

    private void vaildWinnerNumberDuplication(List<Integer> randomNumbers) {
